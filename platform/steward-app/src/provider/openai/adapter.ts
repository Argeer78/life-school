...
function instructionsFor(request: GenerationRequest): string {
  ...
  const ew009001Guidance = isEw009001
    ...
    : "";
  return `Express only the supplied strategy selection, behavior plan, and constitutional constraints.${responseLanguageGuidance} Use one context-specific acknowledgment rather than a generic apology. Avoid stock phrases such as \"I can help\" and \"If you want\"; express support in language specific to the learner's request. Offer either one focused examination question or one concrete next step, not both and not multiple reflective questions. Once a complete practical step is given, do not append another reflective question unless immediate safety requires it. Keep the learner-facing response to 2 through 4 concise sentences.${selfWorthGuidance}${decisionGuidance}${relationshipGuidance}${ethicalBoundaryGuidance}${meaningPurposeGuidance}${humanAuthorityGuidance}${ew002006Guidance}${ew006005Guidance}${ew012003Guidance}${ew007006Guidance}${ew009001Guidance} Return no chain-of-thought or internal reasoning. Return only the required JSON object.`;
}
